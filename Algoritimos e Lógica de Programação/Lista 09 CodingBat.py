...
def maior_ponta(nums):
  ...
  for i in range(len(nums)):
    if nums[0] == nums[-1]: bn.append(nums[-1])
    elif nums[0]>nums[-1]: bn.append(nums[0])
    else: bn.append(nums[-1])
  return bn
...
def squirrel_play(temp, is_summer):
  if is_summer==True and 60<=temp<=100: return True
  elif is_summer==False and 60<=temp<=90: return True
  else: return False
