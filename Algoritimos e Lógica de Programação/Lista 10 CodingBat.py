...
def near_ten(n):
  if n % 10 <= 2 or n % 10 >= 8: return True
  else: return False
...
def lone_sum(a, b, c):
  ...
  if a==c: return b
  if b==c: return a
  if not a==b==c or a==b or a==c or b==c: return a+b+c
...
def lucky_sum(a, b, c):
  ...
  if a==13:
    ...
    bn.append(a), bn.append(b)
    return sum(bn)
  else:
    bn.append(a),bn.append(b),bn.append(c)
    return sum(bn)
...
def count_code(s):
  ...
  for i in range(0,len(s)-3):
    if s[i:i+2]=='co' and s[i+3]=='e': bn+=1
  return bn
...
def has22(nums):
  ...
  for i in range(len(nums)-1):
    if nums[i]==2 and nums[i+1]==2: bn.append(nums[i]),bn.append(nums[i+1])
  if sum(bn)==4: return True
  else: return False
...
def soma_na_lista(n, lista):
	for bn in range(len(lista)):
		for i in range(len(lista)):
			if bn != i and lista[bn] != lista[i] and lista[bn] + lista[i] == n:
				return True
	return False
